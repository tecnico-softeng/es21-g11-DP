...
public abstract class LatexVisitor implements Visitor {
    ...
    @Override
    public void visitQuestionDetails(PCIQuestion question) {

        this.result += "\\begin{lstlisting}\n";
        question.visitPCIItems(this);
        this.result += "\\end{lstlisting}\n";

        this.result = this.result + "% Answer: \n\\begin{lstlisting}\n" +
                question.getItemGroupA()
                        .stream()
                        .map(PCIItem::getContent
                        ).collect(Collectors.joining("\n")) +
                question.getItemGroupB()
                        .stream()
                        .map(PCIItem::getContent
                        ).collect(Collectors.joining("\n")) + "\n\\end{lstlisting}\n";
        this.result = this.result + "\\end{ClosedQuestion}\n}\n\n";
    ...
    @Override
    public void visitPCIItem(PCIItem pciItem) {
        this.result += "\t\\PCIItem{" + pciItem.getContent() + "}\n";
        pciItem.visitItems(this);
    }
